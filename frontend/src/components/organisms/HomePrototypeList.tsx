...
import { listPrototypesPaged } from '@/services/prototype.service'
...
import { getPrototypeLastViewedMap } from '@/utils/prototypeLastViewed'
...
const HOME_PROTOTYPE_LIST_FIELDS = [
  'model_id',
  'name',
  'visibility',
  'image_file',
...
const HomePrototypeList = ({
  ...
  const { isOpen: infoOpen, setOpen: setInfoOpen } = useGlobalTooltip(
    'prototype-list-info',
  )
  ...
  const baseParams = useMemo(
    () => ({
      ...
      fields: HOME_PROTOTYPE_LIST_FIELDS,
  ...
  const totalResults = page0Query.data?.totalResults

  const additionalPageIndices = useMemo(() => {
    if (userLoading || totalResults === undefined || totalResults === 0) {
      return []
    }
  ...
  const additionalPageQueries = useQueries({
    queries: additionalPageIndices.map((pageIndex) => ({
      ...
      enabled:
        ...
        pageIndex * itemsPerView < totalResults,
    })),
  })
  ...
  // Flat client-sorted items for 'last-viewed' / 'first-viewed' modes.
  // The server returns items sorted by createdAt:desc; here we re-sort the
  // fetched pages by the localStorage view timestamps.
  const sortedFlatItems = useMemo<(Prototype | undefined)[]>(() => {
    if (sortBy !== 'last-viewed' && sortBy !== 'first-viewed') return []
    const viewedMap = getPrototypeLastViewedMap()
    const pageCount =
      totalResults !== undefined ? Math.ceil(totalResults / itemsPerView) : 1
    const defined: Prototype[] = []
    let undefinedCount = 0
    for (let p = 0; p < pageCount; p++) {
      const page = pagesCache[p]
      for (let j = 0; j < itemsPerView; j++) {
        const item = page?.[j]
        if (item) defined.push(item)
        else undefinedCount++
      }
    }
    defined.sort((a, b) =>
      sortBy === 'last-viewed'
        ? (viewedMap[b.id] ?? -Infinity) - (viewedMap[a.id] ?? -Infinity)
        : (viewedMap[a.id] ?? Infinity) - (viewedMap[b.id] ?? Infinity),
    )
    return [...defined, ...Array(undefinedCount).fill(undefined)]
  }, [sortBy, pagesCache, totalResults, itemsPerView])
  ...
  const isInitialLoading = page0Query.isLoading
