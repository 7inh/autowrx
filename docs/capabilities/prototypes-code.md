...
- When an **owner** views the project editor with `ALLOW_ADDING_FILES` disabled at **Project editor (`/model/:id/library/prototype/:pid/code`)**, the create/import toolbar is hidden and the file tree offers no context menu, drag & drop, or rename/delete; when it is enabled, all of those are available.
...
- `ALLOW_ADDING_FILES=true` → file create/rename/move/delete, drag & drop and ZIP import are offered; default `false` makes the tree read-only (file contents stay editable).
